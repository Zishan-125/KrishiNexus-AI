...
export default function FarmerPortal({ 
  ...
  // AI Crop Doctor (শস্য চিকিৎসক) Local Feature States
  ...
  // AI Crop Doctor Binary Stream Reader
  const handleCropImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    if (file.size > 5 * 1024 * 1024) {
      setAiError("চিত্রটি ৫ মেগাবাইটের (5MB) চেয়ে ছোট হতে হবে।");
      if (e.target) e.target.value = ""; // Flush value
      return;
    }
    ...
    reader.onerror = () => {
      setAiError("ছবিটি রিড করতে সমস্যা হয়েছে। দয়া করে আবার চেষ্টা করুন।");
    };
    reader.readAsDataURL(file);
  };

  // Safe manual zone trigger
  const handleZoneClick = () => {
    if (fileInputRef.current) {
      fileInputRef.current.value = ""; // Clear string data to ensure change handler re-triggers
      fileInputRef.current.click();
    }
  };

  // API dispatcher targeting Next.js route engine
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="glass-card p-5 bg-gradient-to-br from-agri-500/10 to-slateforest-800/60 border-agri-500/20">
          ...
          <h2 className="text-3xl font-display font-extrabold text-white tracking-wide font-mono">
            {(walletBalance?.toLocaleString() ?? "0")} <span className="text-lg font-sans font-normal text-slateforest-400">BDT</span>
          </h2>
        ...
        <div className="glass-card p-5 bg-gradient-to-br from-market-gold/10 to-slateforest-800/60 border-market-gold/20">
          ...
          <h2 className="text-3xl font-display font-extrabold text-white tracking-wide font-mono">
            {(pendingEscrow?.toLocaleString() ?? "0")} <span className="text-lg font-sans font-normal text-slateforest-400">BDT</span>
          </h2>
          <p className="text-[10px] text-slateforest-400 mt-1">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 items-start">
        ...
        {/* LEFT COMPONENT (Span 2): Harvest Workflows */}
                  ...
                  <div key={listing.id} className="glass-card p-5 flex flex-col gap-4 bg-slateforest-850/40 relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-3 gap-2 bg-slateforest-950/40 p-3 rounded-xl border border-slateforest-800 text-xs font-mono text-center">
                      ...
                      <div>
                        ...
                        <span className="font-bold text-white">{listing.totalAmount?.toLocaleString() ?? "0"}</span>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {/* RIGHT COLUMN (Span 1): AI Crop Doctor and Orders */}
        <div className="flex flex-col gap-6">
          
          {/* AI CROP DOCTOR (শস্য চিকিৎসক) INTERACTIVE PORTAL */}
          <div className="glass-card p-5 bg-gradient-to-b from-slateforest-900 via-slateforest-900 to-slateforest-950/90 border border-agri-500/30 shadow-2xl flex flex-col gap-4">
            ...
            {/* Hidden Input File Element without mobile blocking capture tags */}
            <input 
              type="file" 
              accept="image/*" 
              ref={fileInputRef}
            ...
            {/* Action Trigger Analysis Controls */}
            {cropImage && !cropDiagnosis && (
              ...
              >
                {aiLoading ? (
                  ...
                  <>
                    <Sparkles className="w-4 h-4" /> রোগ নির্ণয় করুন (Run AI Analysis)
                  </>
                )}
              </button>
            )}

            {/* Diagnosis Report Sheet View Panel */}
            {cropDiagnosis && (
                  ...
                  <div className="flex justify-between items-start border-b border-slateforest-800 pb-3">
                    ...
                    <div className="text-right">
                      <span className="font-mono font-bold text-agri-400 text-sm">
                        {order.totalAmount?.toLocaleString() ?? "0"} BDT
                      </span>
                    </div>
                  </div>
